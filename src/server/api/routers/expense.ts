  ...
  overview: orgProtectedProcedure.query(async ({ ctx }) => {
    ...
    const [result] = await ctx.db
      .select({
        ...
        sharedTotal: sql<number>`coalesce(sum(case when ${expenses.isShared} = true then ${expenses.amount} end) / (${memberCountSubQuery}), 0)::float`,
        personalTotal: sql<number>`coalesce(sum(case when ${expenses.isShared} = false then ${expenses.amount} end), 0)::float`,
